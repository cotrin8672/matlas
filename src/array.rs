...
impl<'mex> Matlab<'mex> {
    ...
    pub fn string(&self, value: &str) -> Result<OwnedArray<'mex>> {
        ...
        let dimensions = [1, utf16.len()];
        let raw = unsafe { ffi::matrust_create_char(dimensions.len(), dimensions.as_ptr()) };
        let raw = NonNull::new(raw).ok_or_else(|| Error::allocation("create string"))?;
        let mut array = unsafe { OwnedArray::from_raw(raw) };
        array.as_mut().chars_mut()?.copy_from_slice(&utf16);
        Ok(array)
    }
    ...
    pub fn sparse<T: Numeric>(
        ...
        let raw = unsafe {
            ffi::matrust_create_sparse(rows, columns, values.len(), 0, i32::from(T::COMPLEX))
        };
        let raw = NonNull::new(raw).ok_or_else(|| Error::allocation("create sparse array"))?;
        let array = unsafe { OwnedArray::from_raw(raw) };
        if array.as_ref().class() != Some(Class::Double) || T::CLASS != Class::Double {
            return Err(Error::new(
                ErrorKind::InvalidInput,
                "create sparse array",
                "MATLAB's published sparse constructor supports f64/Complex<f64>; use logical_sparse for bool",
            ));
        }
        unsafe {
